...
async function evaluateW1(sb: any, rules: Record<string, RuleConfig>): Promise<AlertCandidate[]> {
  ...
  for (const userId of userIds) {
    ...
    if (rules["W1_STREAK"] && daysSinceLast >= (rules["W1_STREAK"].threshold.inactive_days ?? 3)) {
      // daysSinceLast bisa berupa Infinity (belum pernah mencatat aktivitas
      // sama sekali) -- jangan pernah tampilkan angka mentahnya di body,
      // karena Infinity di-convert ke string literal jadi teks "Infinity"
      // yang tidak masuk akal buat user.
      const progressPhrase = Number.isFinite(daysSinceLast)
        ? `Kamu sudah ${daysSinceLast} hari belum mencatat progres skripsi.`
        : "Kamu belum pernah mencatat progres skripsi sama sekali.";
      candidates.push({
        user_id: userId,
        goal_id: null,
        rule_code: "W1_STREAK",
        title: "Sudah beberapa hari tanpa progres skripsi",
        body: `${progressPhrase} Semakin lama ditunda, semakin besar risiko keteteran mengejar deadline. Lanjutkan lagi sekarang, sesi singkat juga tetap dihitung.`,
        deep_link: "/monitoring",
        // Simpan null (bukan Infinity) di meta -- Infinity tidak valid di JSON
        // dan akan gagal/ter-null-kan diam-diam saat JSON.stringify.
        meta: { days_since_last: Number.isFinite(daysSinceLast) ? daysSinceLast : null },
      });
